...
extern "C" void idt_init(void) {
	...
	unsigned long intyield_address;   
	...
	divide_by_zero_address = (unsigned long)divide_by_zero;
	...
	double_fault_address = (unsigned long)double_fault;
	IDT[8].offset_lowerbits = double_fault_address & 0xffff;
	IDT[8].selector = 0x08; /* KERNEL_CODE_SEGMENT_OFFSET */
	IDT[8].zero = 0;
	IDT[8].type_attr = 0x8e; /* INTERRUPT_GATE */
	IDT[8].offset_higherbits = (double_fault_address & 0xffff0000) >> 16;

	protection_fault_address = (unsigned long)protection_fault;
	IDT[13].offset_lowerbits = protection_fault_address & 0xffff;
	IDT[13].selector = 0x08; /* KERNEL_CODE_SEGMENT_OFFSET */
	IDT[13].zero = 0;
	IDT[13].type_attr = 0x8e; /* INTERRUPT_GATE */
	IDT[13].offset_higherbits = (protection_fault_address & 0xffff0000) >> 16;
	...
	intyield_address = (unsigned long)intyield;
	IDT[0x79].offset_lowerbits = intyield_address & 0xffff;
	IDT[0x79].selector = 0x08; /* KERNEL_CODE_SEGMENT_OFFSET */
	IDT[0x79].zero = 0;
	IDT[0x79].type_attr = 0x8e; /* INTERRUPT_GATE */
